...
| K3 | 各目标错误总数 | kotlin f32 103 / f64 114；rust f32 2824 / f64 2821；ts 330；dart 317（生成侧 269、测试侧 48）；swift f32 161 / f64 142（WMO 测量，TiqianEngine import 消融后真值；未消融时该 import 失败掩蔽全部后续诊断，实测仅 1 为假值）。全部为 census18 实测值（boring `31e39bf0` × tiqian `3f609c0f`） | 全部 0 | 各节逐类表求和（完整性数字，非派发单位） |
