...
| K3 | 各目标错误总数 | kotlin f32 64 / f64 64；rust f32 1913 / f64 1900；ts 16；dart gen 31 / tests 0；swift f32 0 / f64 0（strip import 联合 WMO）。全部为 census26 实测值（2026-09-12，boring `a0069187` × tiqian `d0a2ab9a`）；census20 旧值（228/239 等）仅在追溯时引用 | 全部 0 | 各节逐类表求和（完整性数字，非派发单位） |
