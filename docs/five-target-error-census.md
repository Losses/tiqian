...
| K3 | 各目标错误总数 | kotlin f32 228 / f64 239；rust f32 2741 / f64 2738；ts 330；dart 135（生成侧 96、测试侧 39）；swift f32 11 / f64 5（WMO 测量，TiqianEngine import 全消融真值；未消融时 import 失败掩蔽全部后续诊断，实测仅 1 为假值）。全部为 census20 实测值（boring `464c54c2` × tiqian `3f609c0f`） | 全部 0 | 各节逐类表求和（完整性数字，非派发单位） |
