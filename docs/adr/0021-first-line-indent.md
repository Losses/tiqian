...
> 下文 `firstLineIndentEm` 是决策实现时的历史名称。

## Context

CLREQ：「段首缩排以两个汉字的空间为标准。若遇到杂志等多栏排版……时有改用
缩排一字」；「若首行行首出现开始夹注符号，可以缩减该符号始侧二分之一个
汉字大小的空白」。此前以整段为单位的特性完全缺失（gap audit 第 5 项）。
...
- justify 端：首行的对齐目标同样是缩小后的行宽。
- 结果端：`LineBox.indent` 暴露行首 inline 方向起点偏移；宽度字段不含
...
缩进 2em 后紧跟已削前空白的开括号，字面前视觉空白正好为 2em，CLREQ 的
