...
提椠的早期实现已经覆盖字体 fallback、CJK 度量规整、CLREQ 推荐码点、PunctuationAtom、SpacingPlan、初步 kinsoku repair 与 justification。继续向 PushIn、实际 shaping、平台 renderer 前进之前，需要把项目的中文排版思维约束写清楚，避免后续 agent 把实现改回“西文文本引擎 + 中文特例补丁”的形状。
...
所有行内空间调整都应优先消费结构化 glue，不采用写在代码各处的 `advance -= 0.5em`。
...
- API 设计会逐步出现 em / grid / line occupancy 语义，不只暴露 px。
- Playground 必须能显示 natural grid、adjusted geometry、spacing decisions 和 line decisions。否则它只能证明代码跑了，不能证明排版判断合理。
- Slice 4b 的 PushIn 不能写成“把标点放回上一行”的特例。它必须查询 `SpacingPlan` / `AdjustmentOpportunity`，确认当前行有足够可压缩 glue，才能选择 PushIn。
...
- 行长字号整数倍量化已实现，见 ADR 0028（`LineLengthGridQuantization` /
  `GridBodyAlignment`），是 grid-first 的「行长」部分。剩下的「行内内容凑整」
...
- shaping adapter 接入后，用实际 glyph ink bounds 改进 `PunctuationAtomBuilder`。
