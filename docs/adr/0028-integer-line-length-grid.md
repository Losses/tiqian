...
响应式 / 实际容器宽度几乎不会正好是字号的整数倍（200px / 16px = 12.5 字）。
于是每行都靠 justify 的均匀字距把半个字的零头摊掉，正文并不实际落在字格上。
...
不足一字的余量 `slack = maxWidth − measure ∈ [0, fontSize)` 不进版心，它
用来在容器内摆放**整块正文**。
...
实现为 `ParagraphStyle.lineLengthGrid: LineLengthGrid`：
...
**默认开启**：「我们只能向下取整」是引擎在响应式下的常态，它不属于可选项；
...
- 容器宽度正好为字号整数倍时（fixture 240/320/…），`slack = 0`、`bodyOffset
  = 0`、`measure = maxWidth`，与旧行为完全一致，这些 golden 输出不变。
- 非整数倍的 fixture（220→208/13字、200→192/12字、100→96/6字、…）版心
  缩小到整字：纯汉字行此时**正好填满字格、justify 余量为 0**，inter-char
  均匀字距消失。这正是 grid-first 想要的：正文落在格上，不靠字距凑整。
  golden 已按此重生成。
...
- `LayoutResult.size.width` 仍为内容实测宽度 coerce 到容器（含 bodyOffset），
  语义不变。
- 竖排：取整与 body 偏移沿 inline 方向定义；竖排时 inline 方向变为纵向（block
