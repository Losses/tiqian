...
3. **悬挂的收益集中在窄行（≤ ~12–16 字）**：把无法修复的行消除、把整字
   推出（CarryPrevious）减半；典型行的平均字距几乎不变，收益在尾部。
...
- **> 24 GB、> 32 严格**：是「宽行能负担更严禁则」的取舍，实验证明的是
  更严档在宽行**变便宜**，不会变得更好（严格唯一效果是禁破折号/省略号居
...
CLREQ 主张「一份文档内禁则档位应统一」；自适应是面向响应式 / 移动端
重排（measure 随容器变）的现代扩展，并不违背该主张：同一容器宽度下全段一致。
...
- repair-mechanism 微 fixture 与单测 pin `Fixed(Basic)`（`pinBasicNoHang`），
  与 pin `firstLineIndentEm = 0` 同理，避免自适应遮蔽被测的具体修复。
- 合并清理：删除从未接入使用的旧 `HangingPunctuationPolicy` 预留枚举与
