...
- **频带切换补丁（`SparseBandPatch`）**。页面只保留一份 HTML（构建期写入静态页的
  ...
  renderArtifactSha256）与结构走查发生在 connect 与每次切换频带。它们是与宿主页面
  ...
  增量补丁。
- **运行时回填**。root 自带一份 DataView 作带缓存：引擎切换频带的结果在应用前编码为
  ...
  重算频次从每次切换频带降为每带首次访问（每段落每 digest 生命周期上界为可达带
...
- **失效合一**。失效与切换频带是同一条管线的两种粒度：条目来源（构建期表、带
...
- **闲时预热**。回填的填充顺序从访问驱动改为闲时驱动：coordinator 以最低优先级
  ...
  闲时。预热单元与频带切换 miss 走同一执行装置。闲时预热随 `MeasureIntervalPlanTable`
  整体受开工条件约束。
- **开工条件**。四项 bench（时序 golden 任务的度量集），各自记录绝对值与对比
  ...
  （对比 `AtomicParagraphDomSwap` 路径，含每次切换频带的结构走查与 digest 重算
...
px 覆盖层在客户端运行。横向路径把频带切换成本降为查表与补丁后，性能余量足够，装饰
...
- 频带切换成本从「引擎重算 + 整段重建」变为「查表 + 原地补丁」，拖动场景
  （120Hz 单帧 8.3ms 预算）的重复频带切换成本下降；代价是构建期新增可达带全集的
  断行计算与表体积。
- 运行位置合一与失效合一：重算只剩每带首次访问且无 plan 序列化往返；Worker
  ...
  与开工 bench 的首访成本项）；失效与切换频带共用同一条回填管线，digest 变更
  换新 DataView 实例，prepared-dom 的运行时渲染路径退到构建期。
- 带内宽度变化不再进入频带切换路径：断点与拉伸分配不变，bodyAlignment 为左对齐时
  块偏移为 0、无 DOM 写入，非左对齐时重写一次块偏移。
- 框架集成边界不变（ADR 0042）：宿主框架拥有根元素，引擎拥有根内子树；切换频带对
...
不为未发生的频带切换付永久字节；紧凑编码后全带预计算的代价才可接受（见 Decision）。

### 带格式表按某份 HTML 的偏移编码

拒绝。频带切换补丁的坐标基准必须是 source：跨行的 span 拆分与语义元素克隆随带变化，
...
2. `GridQuantizationUnconditional` 实施：`lineLengthGridEnabled: false` 的旧
   ...
   字段删除后 Rust 侧测试全部通过。
3. `MeasureIntervalPlanTable` 开工前完成四项 bench 并记录数据；实施后频带切换路径
   以时序 golden 记录（断点表命中、补丁作用范围、翻转计数、闲时预热后的引擎
   调用计数）。
4. 频带切换补丁等价：采样若干带迁移，断言补丁后 DOM 与同格数整段重建
...
- [ ] **54-9 频带切换补丁**（`SparseBandPatch`）：source 到活 DOM 跳表、两份断点向量
...
- [ ] **54-11 失效合一**：digest 变更换新 DataView，填充前读取按 miss 处理；
  ...
  五表文本。
  KPI：运行时切换频带与重建共用条目这一种几何输入；plan 对象运行时消费范围 0。
...
| 指标 | 基线 | 度量 |
|---|---|---|
| 频带切换路径 | 引擎重算加整段重建 | 查表加原地补丁；时序 golden 记录频带切换帧耗时与 mutation 数 |
...
| 回填命中 | 无既有数据 | sveltekit 站点与 astro 站点频带切换命中率 |
