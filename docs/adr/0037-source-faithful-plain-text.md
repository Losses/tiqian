...
- **默认无首行缩进**：作者没缩进就不缩进。首行缩进/段间距是 `ParagraphStyle` 的 opt-in，给正式排版用；纯文本默认不开启。

段内**强制换行**(`<br>` 那种「不分块只换行」)是另一回事、且在中文正文罕见，本 ADR 不为它引入独立概念：`\n` 一律按硬断行处理。

### 2. `ZeroWidthSpaceSoftBreak`：U+200B 是结构控制，不属于坏字形
...
也不能因为 `measureText()` 正常返回 `0` 而把整段判为字体失败。
...
在引擎内实现（干净模块 `linebreak` + `layout`，不依赖 compose）：
...
Compose 入口用 `CjkText` 直接接入；public `CjkParagraph` 不保留兼容别名。Compose
模块内部可以保留一个私有/`internal` layout node，但它不是作者 API。

## Alternatives considered

- **`\n\n` 才成段（Markdown）**：与中文写作相悖，会把整篇合并成少数巨段。否。
- **`CjkDocumentText` 严格文档子类型**（前空两格等）：是多余的，首行缩进/段间距是 `ParagraphStyle` 参数，不属于新类型。否。
- **在 compose 层把 `\n` 切成多段、各自布局**：最省事，但碎掉单一 `LayoutResult`（跨段选择/无障碍/行栅格断裂），且要改 compose WIP 文件。在引擎内做是单一结果、可查询（见 ADR 0036 的 offset/line/box 查询），且落在干净模块。否（v1 就在引擎内做）。
- **预发布期保留 `@Deprecated` 兼容别名**：库还没有发布过测试版，不需要保留历史兼容。否。
