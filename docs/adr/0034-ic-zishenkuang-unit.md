...
CJK 排版以「字」为天然单位：行长「40 字」、缩进「2 字」、相邻拼音「1/4 字」；CLREQ 通篇如此。但当前作者面 / `ParagraphStyle` 把这些表达成无单位的 `Float`：
...
- Compose 面 `textStyle.fontSize` 是无单位的 px（调用者自乘 density）。

三个新压力把「需要一个正式单位」推到台面：

1. **字身框已被正式度量**（[ADR 0002](0002-script-aware-font-metrics.md)：BASE `ideo/idtp`，缺则 sTypo）：「一个字」有了精确所指，即字体声明的字身进格。
2. **混排存在**（[ADR 0030](0030-rich-text-spans.md)）：「2 em」在行内变号处歧义（谁的 em？）；需要一个**锚点明确**的单位。
3. **Compose 外壳缺 CJK 习惯单位**：无单位的 `Float` px 不符合直觉，也不该让每个调用点自己换算 density。
...
- **以整段为单位的量**（行长、段首/段落缩进、节距、列表标记列宽）→ 锚**段落基准 CJK 字身框**（= 段落 `fontSize`，不随行内变号）。
...
### 实施分期（控制 blast radius）

- **本期(作者面)**：`ParagraphStyle` 的缩进、`CjkBlock.List.indent`、Compose `Float.ic` 扩展改用 `Ic`；以整段为单位的量锚段落 `fontSize`。
- **暂不动(内部已是正确 em 倍数,纯类型化收益、golden 风险高)**：CLREQ profile 内部常量（`gapEm`、kinsoku 阈值 `shortBelowEm/hangBelowEm/...`、标点 `defaultBodyEm/defaultAdvanceEm`）、ruby 渲染常量（`RUBY_*_EM`）。它们**概念上是 `ic`**，但保持 `Float` 内部值;待行内锚点(per-owner)统一处理时再议。

## Mechanism

- `Ic` + `toPx` 落 `core`（平台无关）。
- `ParagraphStyle.blockIndentEm: Float` → `blockIndent: Ic`；`firstLineIndentEm: Float?` → `firstLineIndent: Ic?`。引擎解析缩进的位置 `… × fontSize` 换成 `….toPx(fontSize)`。
...
- 内部常量暂留 `Float`，是已知、有界的不一致；行内锚点统一处理时清。
- 竖排展望：竖排时字身框「进格」是字高方向；`Ic` 计数不变，`toPx` 换竖向字身框即可；锚点规则已留好竖排口。
