...
容器的实际宽度，不能据此宣称已经输出最终提椠断行。固定版心快照也只能作为严格校验后采用的 cache，
不能替代正文 source。
...
增加版本漂移，却不会形成实际独立的能力所有权。独立 npm 包仍使未使用 Svelte 或 Astro 的用户不必
安装相应框架依赖。
...
`prepareFontContract()` 的公共输入不再要求 `maxWidthPx`。Node 内部为运行同一条实际 layout / shaping
...
和无 JavaScript 路径因此在服务器结果上成立；custom element 加载后读取实际 computed typography 与
...
浏览器必须继续验证 live source、字体、typography 与有效行宽，不一致时用 semantic source 走实时布局。
