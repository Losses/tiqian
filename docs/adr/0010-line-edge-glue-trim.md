# ADR 0010: LineEdgeGlueTrim 补全 ADR 0004 的加法模型
...
[real-text-fixture-1 观察 #3](../research/real-text-fixture-1.md) 在实际段落上让这个 gap 显现出来：12 行里每个以 `。` `，` `」` `）` 结尾的行都顶满 1em，行尾视觉宽度偏大。在短 fixture 上看不出来，所以 Slice 0–6a 一路绿灯地把 atom 系统建好，但「行尾半宽」一直挂在 [ADR 0006](0006-hanging-punctuation-opt-in.md) 的 follow-up 上当成「Slice 4b 收尾时再做」。它属于 ADR 0004 本来就该有的语义，不属于新功能。

孔雀计划《中文排版的最大迷思：标点悬挂》明说：**判断悬挂之前必须先做行尾半宽**，否则悬挂只会把不齐换成更不齐。当前模型给 hang 留了 profile opt-in 通道（ADR 0006），却没把作为前提的「严格行尾半角」实现。

## Decision

把 ADR 0004 加法模型的「edge glue 在 line 边缘被消耗」这一基础语义补上，作为 `ParagraphLayoutEngine` 在 lineBreak 之后、justifier 之前的独立步骤：

### 1. GlueBudget：跨步骤共享的 edge glue 预算
...
约束：`s_leading + ... ≤ leadingNatural`，`s_trailing + ... ≤ trailingNatural`。Edge trim 只能消费 `remaining`，自动避免重复计入。
...
`PunctuationTrailing` priority 不能再从已 trim 的 edge 借空间：edge glue 已经消耗完。Justifier 通过同一份 GlueBudget 知道 remaining，自动跳过 zero-remaining 的 atom。无需 justifier 自己改逻辑，只要传 trimmed clusters 进去即可。
...
- **实际文本立刻变窄**：以标点结尾的行省 4px（PauseOrStop / Closing 类）。real-paragraph-1 12 行里至少 5–6 行会显示出差别。
- **测试 baseline 变化**：以下 engine 模块测试断言需要更新，因为它们的预期值是 ADR 0004 补全前的「1em 顶到底」语义：
  ...
  - 其它含标点结尾的可能也会变；统一改。
- **PushIn 测试不变**：PushIn 已经用满 trailing，trim 拿 0，行为一致。
...
- **悬挂（Hang）的前置条件就位**。今后 [ADR 0006](0006-hanging-punctuation-opt-in.md) 的 follow-up 实现 Hang 时，可以直接基于「严格行尾半角」做差分判断，不需要从零起步。

## Alternatives considered

- **跟 PushIn 合并成同一个「TrailingGlueTrim」概念，按 reason 区分**。否决：PushIn 是 kinsoku 修复决策，要进 RepairOption / repair penalty 体系；LineEdgeTrim 不是修复，是渲染阶段的固定语义。合并会让 RepairOption 列表里挤进非修复条目，混淆 line decision 的语义。**两者用同一份 GlueBudget 共享 trailing 库存就够了**，不需要类型上合并。
...
- `effectiveTrailingGlueCapacity = trailingNatural - trailingConsumed` 这套预算同时回答了 [ADR 0006 follow-up](0006-hanging-punctuation-opt-in.md) 里 PushIn capacity 跟 SpacingCompression 重复计入的那个 known 简化：同一份 budget 自动协调。
