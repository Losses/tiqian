...
按六档（ADR 0020）压缩前一行把它放进去。普通断行按自然宽度判定，
...
行宽」，行末按 ADR 0020 六档分配压缩，记为 `PushIn`（reason 前缀
`ShrinkToFitLineFill` 区分于 `ForbiddenAtLineStart`）。greedy /
lookahead / 评分路径同一准入；避头尾修复降为补足手段。
...
- 若将来要重新评估，需先回答：压缩态填满的版面是否真是目标风格。
  那是一个通过 AdjustmentStylePolicy 开启的 opt-in，不属于默认行为。
