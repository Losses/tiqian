...
孔雀计划最有价值的地方并非给出一张可直接编码的规则表；它提供的是中文排版的思维顺序：

1. 先有正文规格、字格、行长和行数，再有版心。
2. 标点并非固定全宽字符；它是字面与空隙的组合。
3. 避头尾并非一个断行布尔规则；它是不可分字段、实际断行、行内调整三件事的协作。
4. 网格是基础工具，不属于牺牲易读性的形式目标。
5. Unicode / CSS / 平台默认规则是重要输入，但不等于中文排版的最终风格。

这组判断与提椠当前方向一致：提椠应该是一个可解释的 CJK 排版物理模型，不用在平台 text layout 之上堆补丁。
...
- fixture 应包含固定字数行长，例如 12em、15em、22em，不能只用随机 px 宽度。
...
《全角半角碎碎念》的核心启发是：中文标点不应只按“全角 / 半角字符”理解。传统活字工艺里，可以使用半身标点，再按需要补二分空、四分空等铅空。因此，标点的宽度来自字面与空隙的组合。
...
- Slice 4b 应优先实现 PushIn；Hang 放在其后。PushIn 要通过压缩当前行可用的 punctuation glue 把避头标点收回行尾。
- CarryPrevious 仍是 PushIn 容量不足时的默认后备处理。它可能牺牲本行字距或留下 ragged 状态，但这是可解释的避头尾修复。
...
- “常规 / 强制”并非简单开关；它要和标点挤压、两端对齐、行长是否为字号整数倍联动。
...
- 为了纵横绝对对齐，把所有标点、数字、西文都强行排成全宽，牺牲现代中文的易读性。
