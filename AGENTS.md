...
## 并行派发纪律（2026-09-08 用户裁定）

外部修复任务的派发渠道与额度：cmd 免费档最多 2 并发（第二席保留给
meituan/longcat-2.0，拉不起属间歇状态，挂持续轮询直到拉起，禁止改派
其他模型或宣布封锁）；zen 免费档最多 1；lingsuan 与 duck 两个中继的
额度各自独立计算，duck 上 terra 与 luna 合计最多 3 且两者都要用。

- **空闲额度零容忍。** 任何渠道有空位而没有在飞任务即执行错误。
  「任务书还没写好」「修复位置还没核对完」「会话上下文紧张」都不构成
  留空理由：核对与任务书是派发方的工作，发现空位的当前最高优先动作
  就是完成核对并派出，然后才回主线。
- **腾位即补。** 并行任务交付或死亡腾出席位后，验收该任务的第一个
  动作就是补位。补位检查绑定到每次交付或死亡通知的处理流程，不依赖
  记起。
- **时限。** 空位出现后 5 分钟内未派即违纪；用常驻监控按渠道计数
  催报实现，不依赖人工巡视。
- **外部封锁例外。** 渠道级 503、账号周限额、provider 单并发限制属
  外部封锁，留档证据后可空置，恢复后立即占用；间歇故障用轮询试探。
