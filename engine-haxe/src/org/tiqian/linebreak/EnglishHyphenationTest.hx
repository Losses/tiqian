...
class EnglishHyphenationTest {
    @:test public static function hyphenatesCommonWordsAtSyllablePoints():Void {
        ...
        TracedAssertions.assertTrue(EnglishHyphenationTestHelpers.hyphenated("international").indexOf("in-ter") == 0, EnglishHyphenationTestHelpers.hyphenated("international"));
    }
...
class EnglishHyphenationTestHelpers {
    public static function hyphenated(word:String):String { final offsets = EnglishHyphenation.enUs().hyphenate(word); final b = new StringBuf(); var i = 0; while (i < word.length) { var j = 0; var found = false; while (j < offsets.length) { if (offsets[j] == i) found = true; j++; } if (found) b.add("-"); b.add(word.charAt(i)); i++; } return b.toString(); }
    public static function withinMargins(values:std.ReadOnlyArray<Int>, left:Int, right:Int, length:Int):Bool { var i = 0; while (i < values.length) { if (values[i] < left || values[i] > length - right) return false; i++; } return true; }
}
