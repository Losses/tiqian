...
class EnglishHyphenation {
    ...
    public static function enUs():Hyphenator {
        if (enUsCache == null) { final parsed = ParseTexHyphenationPatterns.parse(EnglishHyphenationPatterns.load()); enUsCache = new LiangHyphenator(parsed.patterns, parsed.exceptions, 2, 3); }
        return enUsCache;
    }
}
