...
class UnicodeWordCharacterTest {
    ...
    private static function currentTrace():TestTraceRecorder {
        if (testTrace == null) {
            testTrace = new TestTraceRecorder("UnicodeWordCharacterTest");
        }
        return testTrace;
    }
    ...
    public static function lettersAndNumbersAreWordCharactersAcrossScripts():Void {
        currentTrace().section("lettersAndNumbersAreWordCharactersAcrossScripts");
        ...
        while (index < positives.length) {
            ...
            TracedAssertions.assertTrue(UnicodeWordCharacter.contains(codePoint), "U+" + lowerHex(codePoint));
            index += 1;
        }
        ...
        while (index < negatives.length) {
            ...
            TracedAssertions.assertFalse(UnicodeWordCharacter.contains(codePoint), "U+" + lowerHex(codePoint));
            index += 1;
        }
    }
