...
class TextShaperCoverageTest {
    ...
    @:test public static function surrogatePairHandlingInCodePointCount():Void {
        ...
        TracedAssertions.assertEqualsInt(1, one.decisions[0].glyphCount); TracedAssertions.assertEqualsFloat(16.0, one.clusters[0].advance);
        var two = shaper.shape(input(surrogateText([0xD83D, 0xDE00, 0xD840, 0xDC0B])));
        TracedAssertions.assertEqualsInt(2, two.decisions[0].glyphCount); TracedAssertions.assertEqualsFloat(32.0, two.clusters[0].advance);
        var high = shaper.shape(input(surrogateText([0xD83D])));
        TracedAssertions.assertEqualsInt(1, high.decisions[0].glyphCount); TracedAssertions.assertEqualsFloat(16.0, high.clusters[0].advance);
        var invalid = shaper.shape(input(surrogateText([0xD83D, 0x41])));
        TracedAssertions.assertEqualsInt(2, invalid.decisions[0].glyphCount); TracedAssertions.assertEqualsFloat(32.0, invalid.clusters[0].advance);
    }
    @:test public static function shapingInputWithFeaturesAndConstants():Void {
        ...
        TracedAssertions.assertNotNullRendered(TextShaper.UNVERIFIED_DISPLAY_SUBSTITUTION_COVERAGE_ISSUE != null, "'" + TextShaper.UNVERIFIED_DISPLAY_SUBSTITUTION_COVERAGE_ISSUE + "'");
        TracedAssertions.assertNotNullRendered(TextShaper.PLATFORM_MULTI_FACE_STRING_DRAW_ISSUE != null, "'" + TextShaper.PLATFORM_MULTI_FACE_STRING_DRAW_ISSUE + "'");
    }
}
