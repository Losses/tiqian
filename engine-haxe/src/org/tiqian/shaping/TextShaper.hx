...
class ShapingInput {
    ...
    public function new(text:String, range:TextRange, style:TextStyle, fontDecision:FontDecision, ?displayText:Null<String>, ?openTypeFeatures:Array<String>) {
        this.text = text; this.range = range; this.style = style; this.fontDecision = fontDecision;
...
class ShapingResult {
    ...
    public function new(clusters:Array<Cluster>, glyphRuns:Array<GlyphRun>, ?decisions:Array<ShapingDecisionInfo>) {
        this.clusters = clusters; this.glyphRuns = glyphRuns; this.decisions = decisions == null ? [] : decisions;
    }
}
...
interface ITextShaper { function shape(input:ShapingInput):ShapingResult; }

enum ShapingSource { Stub; JvmAwt; AndroidPaint; Skia; HarfBuzz; CoreText; }
...
class ExplainableStubTextShaper implements ITextShaper {
    ...
    private static function nominalAdvanceEm(source:String, displayText:String):Float {
        if (source == "\u2E3A" || displayText == "\u2E3A") return 2.0;
        if (source.length > 0) {
            var allSpaces = true; var i = 0;
            while (i < source.length) { if (source.charCodeAt(i) != 0x20) allSpaces = false; i++; }
            if (allSpaces) return 0.5 * source.length;
        }
        var a = codePointCount(source); var b = codePointCount(displayText);
        return (a > b ? a : b);
    }
    private static function codePointCount(value:String):Int {
        ...
        while (index < value.length) {
            ...
            if (code >= 0xD800 && code <= 0xDBFF && index + 1 < value.length) {
                ...
                if (next >= 0xDC00 && next <= 0xDFFF) index += 2; else index++;
            } else index++;
            count++;
        }
        return count;
