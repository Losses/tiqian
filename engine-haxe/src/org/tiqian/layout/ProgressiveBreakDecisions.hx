...
class ProgressiveBreakDecisions {
    ...
    private static function progressiveBreakPriorityForLine(lineStart:Int, overflowAt:Int, active:ProgressiveBreakOpportunity, opportunities:SortedMap<Int,ProgressiveBreakOpportunity>, adjustedClusters:Null<Array<Cluster>>, lineLimit:Float, cjkInterCharBoundaries:SortedSet<Int>, maxCjkStretchPerGap:Float, sinoWesternBoundaries:SortedSet<Int>, sinoWesternStretchCap:Float):Int {
        final priorities:Array<Int>=[]; var i=lineStart+1; while(i<=overflowAt){final o=opportunities.get(i);if(o!=null&&o.spanRange.start==active.spanRange.start&&o.spanRange.end==active.spanRange.end&&priorities.indexOf(o.tier.priority)<0)priorities.push(o.tier.priority);i++;} var q=1;while(q<priorities.length){final key=priorities[q];var w=q;while(w>0&&priorities[w-1]>key){priorities[w]=priorities[w-1];w--;}priorities[w]=key;q++;} if(priorities.length==0)return active.tier.priority;
