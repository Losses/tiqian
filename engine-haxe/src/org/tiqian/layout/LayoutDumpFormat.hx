...
class LayoutDumpFormat {
    static function joinInts(a:std.ReadOnlyArray<Int>):String { final b=[]; for (i in 0...a.length) b.push(Std.string(a[i])); return b.length==0?"-":b.join(","); }
    static function joinFloats(a:std.ReadOnlyArray<Float>):String { final b=[]; for (i in 0...a.length) b.push(dumpFmt(a[i])); return b.length==0?"-":b.join(","); }
    ...
    public static function decisionDump(r:LayoutResult, label:String):String {
        ...
        for (i in 0...d.inlineObjectPunctuationAttachmentDecisions.length) { final x:InlineObjectPunctuationAttachmentDecisionInfo=d.inlineObjectPunctuationAttachmentDecisions[i]; o.add("inline-object-punctuation "+x.objectRange.start+"-"+x.objectRange.end+" separator="+x.separatorRange.start+"-"+x.separatorRange.end+" punctuation="+x.punctuationRange.start+"-"+x.punctuationRange.end+" source='"+escapeDumpText(x.punctuationText)+"' collapsed="+dumpFmt(x.collapsedAdvance)+" protected="+x.protectedRange.start+"-"+x.protectedRange.end+" reason="+x.reason+"\n"); }
        ...
        for (i in 0...d.inlineObjectDecisions.length) { final x:InlineObjectDecisionInfo=d.inlineObjectDecisions[i]; o.add("inline-object "+x.range.start+"-"+x.range.end+" advance="+dumpFmt(x.advance)+" ascent="+dumpFmt(x.ascent)+" descent="+dumpFmt(x.descent)+" cluster="+x.clusterIndex+" line="+x.lineIndex+" edges="+(x.leadingUniformStretch?"stretch":"fixed")+"/"+(x.leadingPreferredStretchKind==null?"-":x.leadingPreferredStretchKind)+"/"+dumpFmt(x.leadingPreferredStretchNaturalWidth)+"→"+dumpFmt(x.leadingPreferredStretchTargetWidth)+"/"+dumpFmt(x.leadingPreferredStretchCapacity)+"/"+(x.leadingPreventsLineBreak?"closed":"natural")+"/"+dumpFmt(x.leadingShrinkCapacity)+"/"+dumpFmt(x.leadingLineEndDiscardableAdvance)+".."+(x.trailingUniformStretch?"stretch":"fixed")+"/"+(x.trailingPreferredStretchKind==null?"-":x.trailingPreferredStretchKind)+"/"+dumpFmt(x.trailingPreferredStretchNaturalWidth)+"→"+dumpFmt(x.trailingPreferredStretchTargetWidth)+"/"+dumpFmt(x.trailingPreferredStretchCapacity)+"/"+(x.trailingPreventsLineBreak?"closed":"natural")+"/"+dumpFmt(x.trailingShrinkCapacity)+"/"+dumpFmt(x.trailingLineEndDiscardableAdvance)+" reason="+x.reason+"\n"); }
        ...
        for (i in 0...d.decorationDecisions.length) { final x:DecorationDecisionInfo = d.decorationDecisions[i]; o.add("deco "+x.clusterRange.start+"-"+x.clusterRange.end+" '"+x.sourceText+"' kind="+x.kind+" applied="+ (x.applied ? "true" : "false") +" anchor="+dumpFmt(x.anchorX)+","+dumpFmt(x.anchorY)+" diameter="+dumpFmt(x.dotDiameter)+" reason="+x.reason+"\n"); }
        if (d.lineSpacingDecision != null) { final x:LineSpacingDecisionInfo=d.lineSpacingDecision; o.add("linespacing natural="+dumpFmt(x.naturalHeight)+" requested="+(x.requestedLineHeight==null?"-":dumpFmt(x.requestedLineHeight))+" resolved="+dumpFmt(x.resolvedHeight)+" floor="+dumpFmt(x.spacingFloor)+" applied="+(x.floorApplied ? "true" : "false")+" reason="+x.reason+"\n"); }
        if (d.rubyLineHeightDecision != null) { final x:RubyLineHeightDecisionInfo=d.rubyLineHeightDecision; o.add("rubylineheight mode="+x.mode+" base="+dumpFmt(x.baseLineHeight)+" face="+dumpFmt(x.baseFaceHeight)+" ruby="+dumpFmt(x.rubyExtent)+" available="+dumpFmt(x.availableInterlineSpace)+" maxExtra="+dumpFmt(x.maxExtra)+" extras="+joinFloats(x.lineExtras)+" lines="+joinInts(x.expandedLineIndices)+" reason="+x.reason+"\n"); }
        for (i in 0...d.decorationSegments.length) { final x:DecorationSegmentInfo=d.decorationSegments[i]; o.add("decobox "+x.sourceRange.start+"-"+x.sourceRange.end+" kind="+x.kind+" line="+x.lineIndex+" rect="+dumpFmt(x.left)+","+dumpFmt(x.top)+","+dumpFmt(x.right)+","+dumpFmt(x.bottom)+" open="+(x.openStart?"start":"-")+"/"+(x.openEnd?"end":"-")+" reason="+x.reason+"\n"); }
        for (i in 0...d.rubyDecisions.length) { final x:RubyDecisionInfo=d.rubyDecisions[i]; o.add("ruby "+x.baseRange.start+"-"+x.baseRange.end+" '"+x.text+"' line="+x.lineIndex+" centerX="+dumpFmt(x.centerX)+" baselineY="+dumpFmt(x.baselineY)+" size="+dumpFmt(x.fontSize)+" box="+dumpFmt(x.ascent)+"/"+dumpFmt(x.descent)+" width="+dumpFmt(x.width)+" overhang="+dumpFmt(x.overhang)+" locale="+x.locale+"\n"); }
        for (i in 0...d.bopomofoDecisions.length) { final x:BopomofoDecisionInfo=d.bopomofoDecisions[i]; o.add("bopomofo "+x.baseRange.start+"-"+x.baseRange.end+" '"+x.text+"' line="+x.lineIndex+" locale="+x.locale+"\n"); for (j in 0...x.placements.length) { final p=x.placements[j]; o.add("  "+p.role+" '"+p.text+"' rect="+dumpFmt(p.left)+","+dumpFmt(p.top)+","+dumpFmt(p.width)+","+dumpFmt(p.height)+" draw="+dumpFmt(p.drawX)+","+dumpFmt(p.baselineY)+" size="+dumpFmt(p.fontSize)+"\n"); } }
        if (d.inlineObjectLineHeightDecision != null) { final x:InlineObjectLineHeightDecisionInfo=d.inlineObjectLineHeightDecision; o.add("inlineobjectlineheight base="+dumpFmt(x.baseLineHeight)+" face="+dumpFmt(x.baseFaceAscent)+"+"+dumpFmt(x.baseFaceDescent)+" available="+dumpFmt(x.availableInterlineSpace)+" clearance="+dumpFmt(x.minimumClearance)+" ascents="+joinFloats(x.lineAscents)+" descents="+joinFloats(x.lineDescents)+" extras="+joinFloats(x.lineExtras)+" boundaries="+joinFloats(x.boundaryShiftsAfter)+" trailing="+dumpFmt(x.trailingExtra)+" lines="+joinInts(x.expandedLineIndices)+" reason="+x.reason+"\n"); }
        return o.toString();
    }
