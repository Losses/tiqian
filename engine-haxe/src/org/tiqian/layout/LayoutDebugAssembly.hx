...
class LayoutDebugAssembly {
    private static function repairCandidateToDecisionInfo(
        candidate:RepairCandidate,
        clusters:Array<Cluster>
    ):LineRepairCandidateInfo {
        return new LineRepairCandidateInfo(
            candidate.kind,
            candidate.reasonCode,
            clusters[candidate.offenderClusterIndex].range,
            candidate.penalty,
            candidate.accepted,
            candidate.rejectionReason,
            candidate.targetClusterIndex,
            candidate.carriedClusterIndex,
            candidate.shrink,
            candidate.requiredShrink,
            candidate.availableCapacity
        );
    }

    private static function toPushInAllocations(
        allocations:Array<PushInAllocation>,
        clusters:Array<Cluster>
    ):Array<LineRepairAllocationInfo> {
        final pushInAllocations = new Array<LineRepairAllocationInfo>();
        for (aIdx in 0...allocations.length) {
            final alloc = allocations[aIdx];
            pushInAllocations.push(new LineRepairAllocationInfo(
                clusters[alloc.clusterIndex].range,
                alloc.shrink,
    ...
    private static function repairOptionToDecisionInfo(
        repair:RepairOption,
        clusters:Array<Cluster>
        ...
        return switch (repair) {
            case PushIn(penalty, reason, offenderClusterIndex, allocations, totalShrink, totalAvailableCapacity):
                ...
                new LineRepairDecisionInfo(
                    "PushIn",
                    reasonCode,
                    clusters[offenderClusterIndex].range,
                    penalty,
            ...
            case CarryNext(penalty, _, movedClusterIndex):
                new LineRepairDecisionInfo(
                    "CarryNext",
                    "ForbiddenAtLineEnd",
                    clusters[movedClusterIndex].range,
                    penalty,
                    null,
                    movedClusterIndex
                );
    ...
    private static function progressiveBreakTierName(tier:org.tiqian.layout.ProgressiveBreakDecisions.ProgressiveBreakTier):String {
        if (tier == org.tiqian.layout.ProgressiveBreakDecisions.ProgressiveBreakTier.Whitespace) return "Whitespace";
        if (tier == org.tiqian.layout.ProgressiveBreakDecisions.ProgressiveBreakTier.Structural) return "Structural";
        if (tier == org.tiqian.layout.ProgressiveBreakDecisions.ProgressiveBreakTier.Syllable) return "Syllable";
        if (tier == org.tiqian.layout.ProgressiveBreakDecisions.ProgressiveBreakTier.WholeToken) return "WholeToken";
        if (tier == org.tiqian.layout.ProgressiveBreakDecisions.ProgressiveBreakTier.Emergency) return "Emergency";
        return "Unknown";
    }
    ...
    public static function buildLayoutDebugInfo(
        ...
        for (fIdx in 0...stage.fontDecisions.length) {
            ...
            final substitution = org.tiqian.layout.ContextualPunctuationDisplaySubstitution.ContextualPunctuationDisplaySubstitutionFns.substituteForRole(
                stage.punctuationGlyphSubstitutor, clusterText, decision.role);
            ...
            fontDecisionsOut.push(new FontDecisionInfo(
                decision.range,
                clusterText,
                displayText,
                Type.enumConstructor(decision.role),
        ...
        for (mIdx in 0...stage.metricDecisions.length) {
            ...
            metricDecisionsOut.push(new MetricDecisionInfo(
                decision.range,
                decision.sourceText,
                Type.enumConstructor(decision.request.role),
                decision.request.fontKey,
                decision.rawMetrics.ascent,
                decision.rawMetrics.descent,
                decision.rawMetrics.leading,
                Type.enumConstructor(decision.rawMetrics.source),
                decision.layoutMetrics.ascent,
                decision.layoutMetrics.descent,
                Type.enumConstructor(decision.layoutMetrics.baselineClass),
                Type.enumConstructor(decision.layoutMetrics.metricBox),
                Type.enumConstructor(decision.layoutMetrics.source),
        ...
        for (pIdx in 0...stage.punctuationAtoms.length) {
            ...
            punctuationDecisionsOut.push(new PunctuationDecisionInfo(
                atom.range,
                atom.char,
                Type.enumConstructor(atom.punctuationClass),
                atom.advance,
                atom.bodyWidth,
                atom.leadingGlue.natural,
                atom.trailingGlue.natural,
                Type.enumConstructor(atom.anchor),
                atom.inkBounds,
                atom.geometrySource,
                atom.policyBodyFloor,
                atom.inkWidth,
                atom.inkCenter,
                atom.inkContainmentBodyFloor,
                atom.inkContainmentApplied,
                atom.inkBoundsFallback,
                atom.haltAdvance,
                atom.haltValidation,
                atom.advanceExpansion,
                atom.glyphInlineShift,
                atom.glyphPlacementReason,
                atom.leadingGlueInitiallyConsumed,
                atom.trailingGlueInitiallyConsumed
            ));
        }
        ...
        for (sIdx in 0...stage.spacingPlan.adjustments.length) {
            ...
            spacingDecisionsOut.push(new SpacingDecisionInfo(
                adjustment.range,
                adjustment.leftChar,
                adjustment.rightChar,
                adjustment.naturalInnerGlue,
                adjustment.adjustedInnerGlue,
                adjustment.reduction,
                adjustment.reductionTargetRange,
                adjustment.reason
            ));
        }
        ...
        final minLineCount = stage.laidOutLines.length < stage.lineSolution.lines.length
            ? stage.laidOutLines.length
            : stage.lineSolution.lines.length;
        for (lineIndex in 0...minLineCount) {
            ...
            lineDecisionsOut.push(new LineDecisionInfo(
                line.range,
                engine.lineBreaker.strategyName,
                repairName,
                repairPenalty,
                repairDecision,
                repairCandidatesOut,
        ...
        for (i in 0...stage.lineSolution.lines.length) {
            ...
            if (plan != null && (plan.allocations.length > 0 || plan.deficitBefore > 0.0)) {
                ...
                for (aIdx in 0...plan.allocations.length) {
                    ...
                    allocationsOut.push(new JustificationAllocationInfo(
                        stage.clusters[alloc.targetClusterIndex].range,
                        Type.enumConstructor(alloc.kind),
                        alloc.priority,
                        alloc.delta,
                        alloc.reason
                    ));
                }
                justificationDecisionsOut.push(new JustificationDecisionInfo(
                    candidate.sourceRange,
                    plan.deficitBefore,
                    plan.unfilledDeficit,
                    allocationsOut
                ));
            }
        }

        return new LayoutDebugInfo(
            stage.maxLinesDecision,
            metricDecisionsOut,
            stage.geometryDecisions,
            stage.autoSpaceDecisions,
            stage.rubyDecisions,
            stage.bopomofoDecisions,
            fontDecisionsOut,
            stage.shapingDecisions,
            punctuationDecisionsOut,
            spacingDecisionsOut,
            stage.roleOverrideInfos,
            lineDecisionsOut,
            justificationDecisionsOut,
            stage.edgeTrimDecisions,
            stage.decorationDecisions,
            stage.decorationSegments,
            stage.mandatoryBreakDecisions,
            stage.lineSpacingDecision,
            stage.rubyLineHeightDecision,
            stage.inlineObjectLineHeightDecision,
            stage.kinsokuDecision,
            stage.contextualKinsokuDecisions,
            stage.lineLengthGridDecision,
            stage.firstLineIndentDecision,
            stage.inlineBoxDecisions,
            stage.inlineObjectDecisions,
            stage.inlineObjectPunctuationAttachmentDecisions,
            stage.zeroWidthBreakDecisions,
            stage.breakOpportunityDecisions,
