...
class LineOptimizationCoverageTest {
    ...
    @:test public static function hangingClusterIndexPrefersTheHangOffenderOverTheSuffixEnd():Void {
        ...
        final withRepair=LineOptimizationCoverageSupport.line(LineOptimizationCoverageSupport.set([3,4]),RepairOption.Hang(5,"ForbiddenAtLineStart:，:hang",3));
...
class LineOptimizationCoverageSupport {
    public static function start(n:String):Void new TestTraceRecorder("LineOptimizationCoverageTest").section(n);
    public static function set(values:Array<Int>):SortedSet<Int> {
        final b=SortedSet.builder(); var i=0;
        while(i<values.length){ b.put(values[i]); i++; }
        return b.build();
    }
    ...
    public static function movedIndex(o:RepairOption):Int return switch (o) {
    ...
    public static function renderStrategies():String {
        ...
        while(i<values.length){ if(i>0)buf.add(", "); buf.add(Type.enumConstructor(values[i])); i++; }
