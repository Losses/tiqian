...
class ParagraphDpTierPromotionPoolTest {
    @:test public static function foreignSpanCandidateSurvivesThePromotionPoolPurge():Void {
        ...
        TracedAssertions.assertTrue(ParagraphDpTierPromotionPoolTestSupport.repairReason(s.lines[0].repair).indexOf("ProgressiveTechnicalTierPromotion") == 0, Std.string(s.lines));
    }
    @:test public static function committedCompressedLineWithForeignSpanOpportunitiesKeepsPlainPushInReason():Void {
        ...
        TracedAssertions.assertEqualsIntRange(new IntRange(0,2),s.lines[0].clusterRange); TracedAssertions.assertTrue(ParagraphDpTierPromotionPoolTestSupport.repairReason(s.lines[0].repair).indexOf("LineAdjustmentPushIn") == 0);
    }
