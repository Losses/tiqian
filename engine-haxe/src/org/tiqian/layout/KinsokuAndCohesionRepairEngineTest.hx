...
class KinsokuAndCohesionRepairEngineTest {
    @:test public static function bibliographicNumericLocatorExposesStructuralBreaks():Void {
        ...
        TracedAssertions.assertTrue(StringTools.endsWith(lineTexts[0], "44(10):"), "locator should fill the preceding line: " + LineBreakRepairEngineTestSupport.renderStrings(lineTexts));
        TracedAssertions.assertEqualsString("21-38.", lineTexts[lineTexts.length - 1]);
        var noEndOpen = true;
        for (i in 0...lineTexts.length) if (StringTools.endsWith(lineTexts[i], "(")) noEndOpen = false;
        ...
        for (i in 0...lineTexts.length) if (StringTools.startsWith(lineTexts[i], ")")) noStartClose = false;
    ...
    @:test public static function hangingPunctuationFillsLineToMeasureAndOverflowsVisual():Void {
        ...
        final plain = LineBreakRepairEngineTestSupport.fixed(KinsokuLevel.Basic, HangingPunctuationStyle.Disabled).layout(LineBreakRepairEngineTestSupport.input("中文中文，中文。", 64));
        var noneOverflow = true;
        for (i in 0...plain.lines.length) if (plain.lines[i].visualWidth > 64) noneOverflow = false;
        TracedAssertions.assertTrue(noneOverflow);
        var noneHang = true;
        for (i in 0...plain.debug.lineDecisions.length) if (plain.debug.lineDecisions[i].repair == "Hang") noneHang = false;
    ...
    @:test public static function kinsokuFallsBackToLeaveRaggedWhenPreviousLineCannotSpareACluster():Void {
        ...
        TracedAssertions.assertEqualsInt(2, result.lines.length);
        var coffeeText = "";
        for (i in 0...result.clusters.length) if (result.clusters[i].text == "Coffee") coffeeText = result.clusters[i].text;
    ...
    @:test public static function kinsokuLevelNoneLeavesForbiddenMarksAtLineStart():Void {
        ...
        var allNull = true;
        for (i in 0...none.debug.lineDecisions.length) if (none.debug.lineDecisions[i].repair != null) allNull = false;
        TracedAssertions.assertTrue(allNull);
        var anyStart3 = false;
        for (i in 0...none.lines.length) if (none.lines[i].range.start == 3) anyStart3 = true;
        ...
        var anyRepair = false;
        for (i in 0...basic.debug.lineDecisions.length) if (basic.debug.lineDecisions[i].repair != null) anyRepair = true;
    ...
    @:test public static function kinsokuLevelStrictForbidsDashAtLineStart():Void {
        ...
        final basic = LineBreakRepairEngineTestSupport.fixed(KinsokuLevel.Basic, HangingPunctuationStyle.Disabled).layout(input);
        var allNull = true;
        for (i in 0...basic.debug.lineDecisions.length) if (basic.debug.lineDecisions[i].repair != null) allNull = false;
        TracedAssertions.assertTrue(allNull);
        final strict = LineBreakRepairEngineTestSupport.fixed(KinsokuLevel.Strict, HangingPunctuationStyle.Disabled).layout(input);
        var anyRepair = false;
        for (i in 0...strict.debug.lineDecisions.length) if (strict.debug.lineDecisions[i].repair != null) anyRepair = true;
        TracedAssertions.assertTrue(anyRepair);
    }
    ...
    @:test public static function kinsokuPushesLineStartPunctuationIntoPreviousLineWhenTrailingGlueCanShrink():Void {
        ...
        final result = new ExplainableStubParagraphLayoutEngine().layout(new LayoutInput(
            new TiqianTextContent("中文中。"),
            null,
            new ParagraphStyle(null, null, null, Ic.Zero, null, null, new LineLengthGrid(false)),
            new LayoutConstraints(60)
        ));
        ...
        var cSum = 0.0;
        for (i in 0...result.clusters.length) cSum += result.clusters[i].advance;
        TracedAssertions.assertEqualsFloat(56, cSum);
        var gSum = 0.0;
        for (i in 0...result.glyphRuns.length) gSum += result.glyphRuns[i].advance;
        TracedAssertions.assertEqualsFloat(56, gSum);
        var stopAdvance = 0.0;
        for (i in 0...result.clusters.length) if (result.clusters[i].text == "。") stopAdvance = result.clusters[i].advance;
    ...
    @:test public static function lineEndKinsokuMovesDanglingOpenerToNextLine():Void {
        ...
        for (i in 0...result.lines.length) {
            ...
            for (j in 0...result.clusters.length) {
                if (result.clusters[j].range.end <= line.range.end) lastCluster = result.clusters[j];
            }
        ...
        for (i in 0...result.debug.lineDecisions.length) if (result.debug.lineDecisions[i].repair == "CarryNext") anyCarryNext = true;
    ...
    @:test public static function longLatinSentenceWrapsAtWordBoundaries():Void {
        ...
        for (i in 0...result.lines.length) {
            ...
            for (j in 0...first.text.length) if (first.text.charAt(j) != " ") firstAllSpace = false;
            if (firstAllSpace) TracedAssertions.assertEqualsFloat(0, first.advance);
            var lastAllSpace = last.text.length > 0;
            for (j in 0...last.text.length) if (last.text.charAt(j) != " ") lastAllSpace = false;
            if (lastAllSpace) TracedAssertions.assertEqualsFloat(0, last.advance);
        }
    }
    ...
    @:test public static function numberWithSuffixSymbolNeverSplitsAcrossLines():Void {
        ...
        final result = new ExplainableStubParagraphLayoutEngine().layout(new LayoutInput(
            ...
            null,
            new ParagraphStyle(null, null, null, Ic.Zero, null, null, new LineLengthGrid(false)),
            new LayoutConstraints(120)
        ));
        final lineTexts:Array<String> = [];
        for (i in 0...result.lines.length) lineTexts.push(LineBreakRepairEngineTestSupport.lineText(result, i));
        var any50 = false;
        for (i in 0...lineTexts.length) if (lineTexts[i].indexOf("50%") >= 0) any50 = true;
        TracedAssertions.assertTrue(any50, "50% must stay together: " + LineBreakRepairEngineTestSupport.renderStrings(lineTexts));
        var noneEnd50 = true;
        for (i in 0...lineTexts.length) if (StringTools.endsWith(lineTexts[i], "50")) noneEnd50 = false;
    ...
    @:test public static function ordinaryNumericFormsDoNotBecomeBibliographicLocators():Void {
        ...
        for (i in 0...tokens.length) {
            ...
            final result = new ExplainableStubParagraphLayoutEngine().layout(new LayoutInput(
                new TiqianTextContent("中文" + token),
                null,
                new ParagraphStyle(null, null, null, Ic.Zero, null, null, new LineLengthGrid(false)),
                new LayoutConstraints(320)
            ));
            TracedAssertions.assertTrue(result.debug.breakOpportunityDecisions.length == 0, token + " must keep its existing numeric/token policy: " + LineBreakRepairEngineTestSupport.renderList(result.debug.breakOpportunityDecisions));
        }
    }
}
