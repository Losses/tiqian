...
class UnicodePunctuationBoundaryTestSupport {
    ...
    public static function setInts(values:Array<Int>):std.SortedSet<Int> {
        final b=std.SortedSet.builder(); for (i in 0...values.length) b.put(values[i]); return b.build();
    }
    public static function markClosing(i:Int):String return [")", "]", "}", ",", ".", ":", ";", "!", "?"][i];
    public static function markOpening(i:Int):String return ["(", "[", "{"][i];
    public static function layout(text:String, width:Float, b:LineBreaker, ?level:Null<KinsokuLevel>):LayoutResult {
        ...
        for (i in 0...text.length + 1) boundaries.push(i);
        return new ExplainableStubParagraphLayoutEngine(null, null, resolver, null, null, null, null, b, null, null, new NoHyphenator(), null)
            .layout(new LayoutInput(new TiqianTextContent(text, null, boundaries), null,
    ...
    public static function lines(r:LayoutResult, text:String):Array<String> {
        ...
        for (i in 0...r.lines.length) { final x=r.lines[i]; a.push(text.substring(x.range.start, x.range.end)); }
        return a;
    }
    public static function noneStarts(lines:Array<String>, mark:String):Bool { for (i in 0...lines.length) if (StringTools.startsWith(lines[i], mark)) return false; return true; }
    public static function noneEnds(lines:Array<String>, mark:String):Bool { for (i in 0...lines.length) if (StringTools.endsWith(lines[i], mark)) return false; return true; }
    ...
    public static function clusters(text:String, ?latin:Bool):Array<Cluster> {
        ...
        for (i in 0...text.length) a.push(new Cluster(new TextRange(i, i + 1), text.substring(i, i + 1), latin == true ? "latin" : "cjk", 16));
        return a;
    }
    public static function roles(text:String, ?latin:Bool):Array<FontRole> {
        final a:Array<FontRole> = [];
        for (i in 0...text.length) a.push(latin == true ? FontRole.LatinText : FontRole.CjkText);
        return a;
    }
}
...
class UnicodePunctuationBoundaryTestResolver implements ClreqProfileResolver {
    ...
    public function new(level:KinsokuLevel) { this.level = level; }
    public function resolve(_:LayoutProfileId):ClreqProfile {
        final b = ClreqProfile.MainlandHorizontal;
        return new ClreqProfile(b.id, b.strictness, b.region, b.punctuationGlyphPolicy, null, null,
            b.gluePlacement, b.adjustment, KinsokuMode.Fixed(level, HangingPunctuationStyle.Disabled), b.punctuationWidth);
    }
}
