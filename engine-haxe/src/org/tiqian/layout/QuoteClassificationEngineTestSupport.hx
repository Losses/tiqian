...
class QuoteClassificationEngineTestSupport {
    ...
    public static function arm():TestTraceRecorder { return new TestTraceRecorder("QuoteClassificationEngineTest"); }
    ...
    public static function renderRoleMap(map:std.SortedMap<Int,String>):String { var out=""; for(i in 0...map.size()){if(i>0)out+=", ";out+=""+map.keyAt(i)+"='"+map.valueAt(i)+"'";} return "{"+out+"}"; }
    public static function internal(text:String, source:String, role:String, ?section:String):Void {
        var t = section == null ? arm() : begin(section); var r=layout(text,320); var a:Array<RoleOverrideInfo>=[];
        for(i in 0...r.debug.roleOverrides.length){var d=r.debug.roleOverrides[i];if(d.sourceText=="“"||d.sourceText=="”")a.push(d);}
        TracedAssertions.assertEquals(2,a.length); var all=true; for(i in 0...a.length){all=all&&a[i].overriddenRole==role&&a[i].source==source;} TracedAssertions.assertTrue(all);
    }
    public static function fullWidthTest():Void {
        var t=begin("requestsFullWidthCjkQuotesAndSynthesizesTheCellWhenTheFontStaysProportional"); var e=new ExplainableStubParagraphLayoutEngine(null,null,null,null,null,null,null,null,null,null,new ProportionalQuoteTextShaper()); var r=e.layout(input("中“文”中",320)); var o:Null<Cluster>=null;var c:Null<Cluster>=null;
        for(i in 0...r.clusters.length){var x=r.clusters[i];if(x.text=="“")o=x;if(x.text=="”")c=x;}
        TracedAssertions.assertEqualsFloat(16,o.advance);TracedAssertions.assertEqualsFloat(16,c.advance);TracedAssertions.assertEqualsFloat(10,o.glyphInlineShift);TracedAssertions.assertEqualsFloat(0,c.glyphInlineShift);
        var a:Null<PunctuationDecisionInfo>=null;var b:Null<PunctuationDecisionInfo>=null;for(i in 0...r.debug.punctuationDecisions.length){var x=r.debug.punctuationDecisions[i];if(x.char=="“")a=x;if(x.char=="”")b=x;}
        TracedAssertions.assertEqualsFloat(10,a.advanceExpansion);TracedAssertions.assertEqualsString("UnderwidthPunctuationFullWidthBoxPlacement",a.glyphPlacementReason);TracedAssertions.assertEqualsNullableString(null,b.glyphPlacementReason);TracedAssertions.assertEqualsString("InkBoundsFittedBodyCompression",a.geometrySource);TracedAssertions.assertEqualsString("InkBoundsFittedBodyCompression",b.geometrySource);
        var p=LayoutQueries.positionedClusters(r);var po:PositionedCluster=null;var pc:PositionedCluster=null;for(i in 0...p.length){var x=p[i];if(x.range.start==o.range.start&&x.range.end==o.range.end)po=x;if(x.range.start==c.range.start&&x.range.end==c.range.end)pc=x;}TracedAssertions.assertEqualsFloat(po.left+10,po.drawX);TracedAssertions.assertEqualsFloat(pc.left,pc.drawX);var q=e.layout(input("“文",320));TracedAssertions.assertEqualsFloat(8,q.clusters[0].advance);var qp=LayoutQueries.positionedClusters(q);TracedAssertions.assertEqualsFloat(2,qp[0].drawX);TracedAssertions.assertEqualsFloat(8,qp[1].left);
    }

    public static function lineReason(r:LineEndReason):String { var x:String="ParagraphEnd"; switch(r) { case AutoWrap: x="AutoWrap"; case MandatoryBreak: x="MandatoryBreak"; case ParagraphEnd: x="ParagraphEnd"; } return x; }
    public static function startsWith(text:String, range:TextRange, mark:String):Bool { var x=UString.slice(text,range.start,range.end); var r=false; if(x.indexOf(mark)==0) r=true; return r; }
    public static function endsWith(text:String, range:TextRange, mark:String):Bool { var x=UString.slice(text,range.start,range.end); return x.lastIndexOf(mark)==x.length-1; }
}
