...
class LineAdjustmentStageCoverageTestSupport {
    public static function engine(?shaper:ITextShaper, hyphenate:Bool):ExplainableStubParagraphLayoutEngine {
        if (shaper != null) return new ExplainableStubParagraphLayoutEngine(null, null, null, null, null, null, null, null, null, null, shaper);
        if (hyphenate) return new ExplainableStubParagraphLayoutEngine(null, null, null, null, null, null, null, null, null, null, null, EnglishHyphenation.enUs());
    ...
    public static function layout(text:String, width:Float, ?spans:Array<TextSpan>, ?inlineObjects:Array<InlineObjectSpan>,
        ...
        return e.layout(new LayoutInput(new TiqianTextContent(text, resolvedSpans, null, resolvedBreaks), null,
            new ParagraphStyle(null, null, null, new Ic(0.0), null, null, new LineLengthGrid(false)),
            new LayoutConstraints(width), null, null, null, null, resolvedObjects));
    }
    ...
    public static function renderStrings(a:ReadOnlyArray<String>):String {
        final parts:Array<String> = [];
        for (i in 0...a.length) parts.push(a[i]);
        return "[" + parts.join(", ") + "]";
    }
    ...
    public static function renderClusterTextAdvance(a:ReadOnlyArray<Cluster>):String {
        final parts:Array<String> = [];
        for (i in 0...a.length) parts.push(a[i].text + "@" + Std.string(a[i].advance));
        return parts.join(",");
    }
    public static function renderAdvances(a:ReadOnlyArray<Cluster>):String {
        final parts:Array<String> = [];
        for (i in 0...a.length) parts.push(Std.string(a[i].advance));
        return parts.join(",");
    }
    public static function renderClusterTexts(a:ReadOnlyArray<Cluster>):String {
        final parts:Array<String> = [];
        for (i in 0...a.length) parts.push(a[i].text);
        return parts.join(",");
    }
    public static function lineRangeWidths(a:ReadOnlyArray<LineBox>):String {
        final parts:Array<String> = [];
        for (i in 0...a.length) parts.push(a[i].clusterRange.toString() + ":" + Std.string(a[i].naturalWidth) + "/" + Std.string(a[i].adjustedWidth));
        return "[" + parts.join(", ") + "]";
    }
    public static function clusterByText(r:LayoutResult, text:String):Cluster {
        for (i in 0...r.clusters.length) if (r.clusters[i].text == text) return r.clusters[i];
    ...
    public static function trimByReason(r:LayoutResult, reason:String):LineEdgeTrimDecisionInfo {
        for (i in 0...r.debug.lineEdgeTrimDecisions.length) if (r.debug.lineEdgeTrimDecisions[i].reason == reason) return r.debug.lineEdgeTrimDecisions[i];
    ...
    public static function trimsByReason(r:LayoutResult, reason:String):Array<LineEdgeTrimDecisionInfo> {
        ...
        for (i in 0...r.debug.lineEdgeTrimDecisions.length) if (r.debug.lineEdgeTrimDecisions[i].reason == reason) out.push(r.debug.lineEdgeTrimDecisions[i]);
        return out;
    }
    public static function allocationDeltas(r:LayoutResult, kind:String):Array<Float> {
        ...
        for (i in 0...r.debug.justificationDecisions.length) {
            ...
            for (j in 0...allocs.length) if (allocs[j].kind == kind) out.push(allocs[j].delta);
        }
        return out;
    }
    public static function emergencyReasons(r:LayoutResult):Array<String> {
        final out:Array<String> = [];
        for (i in 0...r.debug.emergencyTrackingEligibilityDecisions.length) out.push(r.debug.emergencyTrackingEligibilityDecisions[i].reason);
        return out;
    }
    public static function breakReasons(r:LayoutResult):Array<String> {
        final out:Array<String> = [];
        for (i in 0...r.debug.breakOpportunityDecisions.length) out.push(r.debug.breakOpportunityDecisions[i].reason);
        return out;
    }
    public static function isAllSpaces(s:String):Bool {
        if (s.length == 0) return false;
        for (i in 0...s.length) if (s.charAt(i) != " ") return false;
        return true;
    }
...
class ZeroSpaceShaper implements ITextShaper {
    ...
    public function shape(input:ShapingInput):ShapingResult {
        ...
        for (i in 0...shaped.clusters.length) {
            ...
            clusters.push(LineAdjustmentStageCoverageTestSupport.isAllSpaces(c.text)
                ? new Cluster(c.range, c.text, c.fontKey, 0.0, c.displayText, c.baselineShift, c.leadingLayoutAdvance, c.glyphInlineShift)
                : c);
        }
