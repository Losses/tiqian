...
class PreparedParagraphFns {
    ...
    public static function toPreparedParagraphJson(result:LayoutResult, renderEvidence:Bool = false):String {
        ...
        for (ri in 0...result.glyphRuns.length) { final run=result.glyphRuns[ri]; for (gi in 0...run.glyphs.length) { final glyph=run.glyphs[gi];
            ...
            if (run.openTypeFeatures.length > 0) {
                ...
                for (fi in 0...run.openTypeFeatures.length) { final f = run.openTypeFeatures[fi]; if (fs.indexOf(f) < 0) fs.push(f); }
                featuresB.put(k, fs);
            }
        ...
        final zeroSrc = result.debug.zeroWidthBreakDecisions; for (zi in 0...zeroSrc.length) zeroB.put(rangeKey(zeroSrc[zi].range), true);
        final shapingB = SortedMap.builder(); final shapingSrc = result.debug.shapingDecisions; for (zi in 0...shapingSrc.length) shapingB.put(rangeKey(shapingSrc[zi].range), shapingSrc[zi]);
        final punctB = SortedMap.builder(); final punctSrc = result.debug.punctuationDecisions; for (zi in 0...punctSrc.length) punctB.put(rangeKey(punctSrc[zi].range), punctSrc[zi]);
        final inlineAdvanceB = SortedMap.builder(); final inlineObjSrc = result.input.inlineObjects; for (zi in 0...inlineObjSrc.length) inlineAdvanceB.put(rangeKey(inlineObjSrc[zi].range), inlineObjSrc[zi].advance);
        final edgeStartB = SortedMap.builder(); final edgeEndB = SortedMap.builder();
        final boxSrc = result.input.inlineBoxes;
        for (bi in 0...boxSrc.length) { final b = boxSrc[bi];
    ...
    public static function toPlanWithDiagnosticsJson(result:LayoutResult, renderEvidence:Bool, zeroAdvanceEpsilonPx:Float):String {
        final out=new StringBuf(); out.add("{\"plan\":"); appendJsonString(out,toPreparedParagraphJson(result,renderEvidence)); out.add(",\"diagnostics\":{\"capabilityIssues\":["); var first=true;
        final shapingDiagSrc = result.debug.shapingDecisions; for(di in 0...shapingDiagSrc.length){ final d=shapingDiagSrc[di]; if(d.capabilityIssue!=null){if(!first)out.add(",");first=false;out.add("{\"name\":");appendJsonString(out,d.capabilityIssue);out.add(",\"reason\":");appendJsonString(out,d.reason);out.add(",\"rangeStart\":");out.add(Std.string(d.range.start));out.add(",\"rangeEnd\":");out.add(Std.string(d.range.end));out.add("}");}}
        out.add("],\"advanceSuspects\":[");first=true;for(di in 0...shapingDiagSrc.length){final d=shapingDiagSrc[di];if(!(Math.isFinite(d.advance)&&d.advance>zeroAdvanceEpsilonPx)){if(!first)out.add(",");first=false;out.add("{\"displayText\":");appendJsonString(out,d.displayText);out.add(",\"advance\":\"");out.add(Math.isFinite(d.advance)?ecmaJsonNumber(d.advance):Std.string(d.advance));out.add("\",\"reason\":");appendJsonString(out,d.reason);out.add(",\"rangeStart\":");out.add(Std.string(d.range.start));out.add(",\"rangeEnd\":");out.add(Std.string(d.range.end));out.add("}");}} out.add("]}}"); return out.toString();
    }

    private static function rangeKey(r:TextRange):String return Std.string(r.start)+":"+Std.string(r.end);
    ...
    private static function appendCellEvidence(out:StringBuf, result:LayoutResult, c:Cluster, natural:SortedMap<String,Float>, fonts:SortedMap<String,String>, ids:SortedMap<String,Array<String>>, shaping:SortedMap<String,ShapingDecisionInfo>, punct:SortedMap<String,PunctuationDecisionInfo>, inlineAdvance:SortedMap<String,Float>):Void {
        ...
        var latin=false; { final fdSrc=result.debug.fontDecisions; for(fi in 0...fdSrc.length){ final fd=fdSrc[fi]; if(c.range.start>=fd.range.start&&c.range.end<=fd.range.end&&fd.role==Type.enumConstructor(FontRole.LatinText))latin=true; } } if(latin)out.add(",\"latin\":true");
    ...
    private static function styleAt(result:LayoutResult, offset:Int):TextStyle { var found:TextStyle=null; { final spanSrc=result.input.content.spans; for(si in 0...spanSrc.length){ final span=spanSrc[si]; if(offset>=span.range.start&&offset<span.range.end)found=span.style; } } return found==null?result.input.textStyle:found; }
    ...
    private static function appendParagraphEvidence(out:StringBuf, result:LayoutResult, starts:SortedMap<String,Float>, ends:SortedMap<String,Float>):Void {
        ...
        var emph:Array<DecorationSpan>=[];{final decSrc=result.input.decorations;for(di in 0...decSrc.length){final d=decSrc[di];if(d.kind==DecorationKind.Emphasis)emph.push(d);}}if(emph.length>0){out.add(",\"emphasisRanges\":[");for(i in 0...emph.length){if(i>0)out.add(",");out.add("[");out.add(Std.string(emph[i].range.start));out.add(",");out.add(Std.string(emph[i].range.end));out.add("]");}out.add("]");}
