...
class ParagraphEmptyHyphenShaper implements ITextShaper {
    ...
    public function shape(i:ShapingInput):ShapingResult {
        if (i.text == "-" || i.displayText == "-") return new ShapingResult([], []);
    ...
    public function shape(i:ShapingInput):ShapingResult {
        ...
        if (i.range.length == 2 && i.text.substring(i.range.start,i.range.end) == "em") {
            return new ShapingResult([
                ...
                new Cluster(new TextRange(i.range.start+1,i.range.end),"m","k",10.0,"m")], r.glyphRuns, r.decisions);
        }
        return r;
    }
}
class ParagraphEmptyClusterShaper implements ITextShaper {
    ...
    public function shape(i:ShapingInput):ShapingResult {
        var r = delegate.shape(i);
        if (i.text == "singlecluster" || i.displayText == "singlecluster") return new ShapingResult([],r.glyphRuns,r.decisions);
        return r;
    }
}
...
class ParagraphRollbackShaper implements ITextShaper {
    ...
    public function shape(i:ShapingInput):ShapingResult {
        ...
        var d = new ShapingDecisionInfo(i.range,i.text.substring(i.range.start,i.range.end),i.displayText,"test",1,16.0,"Test","test",null,call == 2 ? 1 : 0,null,null,null,null,null,issue);
...
class ParagraphMultiGlyphShaper implements ITextShaper {
    ...
    public function shape(i:ShapingInput):ShapingResult {
        count++;
        var c = new Cluster(i.range,i.text.substring(i.range.start,i.range.end),"test",32.0,i.displayText);
        var gs:Array<Glyph> = count % 3 == 0 ? [] : (count % 3 == 1 ? [new Glyph(1,i.range,16.0,0.0),new Glyph(2,i.range,16.0,16.0)] : [new Glyph(1,i.range,32.0,0.0)]);
        return new ShapingResult([c],[new GlyphRun(i.range,"test",gs,32.0)]);
    }
}
class ParagraphCoverageHyphenator implements Hyphenator {
    public var mode:Int;
    public function new(?mode:Int=0) this.mode=mode;
    public function hyphenate(w:String):std.ReadOnlyArray<Int> return mode == 1 && w.indexOf("hyphen") >= 0 ? [2,4] : (mode == 2 ? [1,2,3] : [2]);
}
class ParagraphTierHyphenator implements Hyphenator {
    public function new() {}
    public function hyphenate(w:String):std.ReadOnlyArray<Int> return w.indexOf("Machine") >= 0 ? [-1, 0, 3, w.length, w.length + 1] : [2];
}
class ParagraphShapingStageCoverageTestSupport {
    ...
    public static function begin(n:String):TestTraceRecorder { var r = new TestTraceRecorder("ParagraphShapingStageCoverageTest"); r.section(n); return r; }
    ...
    public static function candidate(role:FontRole):FontCandidate return new FontCandidate("k","f",role);
    public static function decision(range:TextRange, role:FontRole):FontDecision return new FontDecision(range,candidate(role),role,"r");
    public static function paragraph(engine:ExplainableStubParagraphLayoutEngine, input:LayoutInput, text:String, width:Float, role:FontRole, ?italic:Bool=false):ParagraphShapingStageResult {
        var range = new TextRange(0,text.length);
        var b = SortedMap.builder(); b.put(range, decision(range,role));
        var rb = SortedMap.builder();
        var cb = [new ResolvedClusterRange(range,role,false,false)];
        return ParagraphShapingStage.shapeParagraph(engine,input,text,16.0,width,cb,b.build(),SortedMap.builder().build(),new org.tiqian.clreq.ClreqPunctuationGlyphSubstitutor(),function(_:Int) return new TextStyle(null,16.0),function(_:Int) return italic,rb.build());
    }
    public static function paragraphRanges(engine:ExplainableStubParagraphLayoutEngine, input:LayoutInput, text:String, width:Float, ranges:Array<ResolvedClusterRange>, decisions:Array<TextRange>):ParagraphShapingStageResult {
        var b = SortedMap.builder(); for (r in decisions) b.put(r, decision(r,FontRole.LatinText));
        var rb = SortedMap.builder();
        return ParagraphShapingStage.shapeParagraph(engine,input,text,16.0,width,ranges,b.build(),SortedMap.builder().build(),new org.tiqian.clreq.ClreqPunctuationGlyphSubstitutor(),function(_:Int) return new TextStyle(null,16.0),function(_:Int) return false,rb.build());
    }
}
