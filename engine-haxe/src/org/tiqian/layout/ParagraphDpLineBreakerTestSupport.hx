...
class ParagraphDpLineBreakerTestSupport {
 ...
 public static function solve(c:Array<Cluster>, width:Float, ?shrink:Array<ShrinkOpportunity>, ?hard:Array<Int>, ?push:Bool, ?ranges:UnbreakableRanges, ?progressive:SortedMap<Int,ProgressiveBreakOpportunity>, ?window:Int, ?cjk:Array<Int>, ?maxStretch:Null<Float>, ?forbidStart:Array<Int>):LineSolution {
  var x=new ParagraphDpLineBreaker(window == null ? 8 : window); return x.breakLines(c,c,width,shrink,ranges,null,null,null,forbidStart==null?null:ints(forbidStart),null,null,ints(cjk==null?[]:cjk),maxStretch==null?Math.POSITIVE_INFINITY:maxStretch,null,null,push,null,ints(hard==null?[]:hard),null,progressive);
 }
 ...
 // cap is 8.
 public static function solveTestDefaults(c:Array<Cluster>, width:Float, ?shrink:Array<ShrinkOpportunity>, ?hard:Array<Int>, ?push:Bool, ?ranges:UnbreakableRanges, ?progressive:SortedMap<Int,ProgressiveBreakOpportunity>, ?forbidStart:Array<Int>):LineSolution {
  var boundaries:Array<Int>=[]; for(i in 1...c.length) boundaries.push(i);
 ...
 public static function rangesString(s:LineSolution):String { var parts:Array<String>=[]; for(l in s.lines) parts.push(l.clusterRange.start+".."+l.clusterRange.end); return "["+parts.join(", ")+"]"; }
}
