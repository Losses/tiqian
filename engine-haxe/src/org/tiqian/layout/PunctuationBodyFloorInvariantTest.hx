...
class PunctuationBodyFloorInvariantTest {
    @:test public static function punctuationNeverResolvesBelowItsBodyWidth():Void {
        ...
        for (text in fixtures) for (maxWidth in widths) {
            final result = engine.layout(new LayoutInput(new TiqianTextContent(text), null,
                new ParagraphStyle(null, null, null, Ic.Zero), new LayoutConstraints(maxWidth)));
            for (i in 0...result.debug.geometryDecisions.length) {
                ...
                TracedAssertions.assertTrue(geometry.resolvedAdvance >= geometry.bodyWidth - 1e-3,
                    "Body floor violated for '" + geometry.sourceText + "' (" + geometry.range.start + "-" + geometry.range.end + ") in \"" + text + "\" @maxWidth=" + maxWidth + ": resolved=" + TestTraceRender.floatText(geometry.resolvedAdvance) + " < body=" + TestTraceRender.floatText(geometry.bodyWidth));
            }
        }
    }
}
