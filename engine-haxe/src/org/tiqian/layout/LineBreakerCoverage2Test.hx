...
class LineBreakerCoverage2Test {
    ...
    @:test public static function testLineCandidateEndsWithProgressiveBreak():Void {
        ...
        final paragraphEnd = new LineCandidate(c.clusterRange, c.sourceRange, c.naturalWidth, c.adjustedWidth, LineEndReason.ParagraphEnd, c.repair, c.repairCandidates, c.hangingClusterIndices);
        ...
        final emptyRange = new LineCandidate(new IntRange(1, 0), c.sourceRange, c.naturalWidth, c.adjustedWidth, c.endReason, c.repair, c.repairCandidates, c.hangingClusterIndices);
...
class LineBreakerCoverage2TestCustomBreaker implements LineBreaker {
    ...
    public function breakLines(n:Array<Cluster>, a:Array<Cluster>, maxWidth:Float, ?s:Array<ShrinkOpportunity>, ?u:UnbreakableRanges, ?i:Float,
        ...
        ?sw:SortedSet<Int>, ?sc:Float, ?p:Bool, ?bias:Float, ?hb:SortedSet<Int>, ?nc:SortedSet<Int>, ?pr:SortedMap<Int,ProgressiveBreakOpportunity>):LineOptimization.LineSolution {
...
class LineBreakerCoverage2TestSupport {
    ...
    public static function oppMap(keys:Array<Int>, opps:Array<ProgressiveBreakOpportunity>):SortedMap<Int,ProgressiveBreakOpportunity> {
        ...
        for (i in 0...keys.length) b.put(keys[i], opps[i]);
        return b.build();
    }
}
