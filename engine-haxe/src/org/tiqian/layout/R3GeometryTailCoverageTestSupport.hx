...
class R3GeometryTailCoverageTestSupport {
    public static function layout(text:String, ?maxWidth:Float, ?maxLines:Int, ?spans:Array<TextSpan>, ?rubySpans:Array<RubySpan>, ?shaper:ITextShaper):LayoutResult {
    ...
    public static function centeredInkShaper():ITextShaper return new CenteredInkShaper();
}
    ...
    public function shape(input:ShapingInput):ShapingResult {
        ...
        for (i in 0...res.glyphRuns.length) {
            ...
            for (j in 0...run.glyphs.length) {
                ...
                glyphs.push(new Glyph(g.id, g.clusterRange, g.advance, g.x, g.y, g.renderFontKey,
                    new Rect(4.0, 2.0, 12.0, 10.0), g.haltAdvance, g.haltPlacementX));
            }
