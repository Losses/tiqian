...
class PreparedParagraphJfTest {
    @:test public static function styleAtAndStyleDeltasInPreparedParagraphJson():Void {
        ...
        final clusters = [
            for (i in 0...6) new Cluster(new TextRange(i, i + 1), ["甲", "乙", "丙", "丁", "戊", "己"][i], "k", i == 1 ? 20 : 16)
        ];
        final glyphs = [for (i in 0...6) new Glyph(i + 1, new TextRange(i, i + 1), i == 1 ? 20 : 16)];
        final result = new LayoutResult(new LayoutInput(content, new TextStyle(), null, new LayoutConstraints(200)), new Size(200, 24), clusters,
            [new GlyphRun(new TextRange(0, 6), "k", glyphs, 100, ["liga", "dlig"])], [
                new LineBox(new TextRange(0, 6), new IntRange(0, 5), 20, 0, 24, 100, 100, 100, null, null, null, null, null, new LineDebugInfo(null))
    ...
    @:test public static function inlineBoxEdgesAndEmphasisDotsFilter():Void {
        ...
        final gs = [new Glyph(1, new TextRange(0, 1), 16), new Glyph(2, new TextRange(1, 2), 16)];
        ...
        ]), new Size(200, 24), cs, [new GlyphRun(new TextRange(0, 2), "k", gs, 32)], [
            new LineBox(new TextRange(0, 2), new IntRange(0, 1), 20, 0, 24, 32, 32, 32, null, null, null, null, null, new LineDebugInfo(null))
        ], new LayoutDebugInfo(null));
    ...
    @:test public static function dashShapingDecisionWithGlyphIds():Void {
        ...
        final cs = [new Cluster(new TextRange(0, 2), "——", "k", 32)];
        final gs = [new Glyph(42, new TextRange(0, 2), 32)];
        final r = new LayoutResult(new LayoutInput(c, null, null, new LayoutConstraints(200)), new Size(200, 24), cs,
            [new GlyphRun(new TextRange(0, 2), "k", gs, 32)], [
                new LineBox(new TextRange(0, 2), new IntRange(0, 0), 20, 0, 24, 32, 32, 32, null, null, null, null, null, new LineDebugInfo(null))
            ], new LayoutDebugInfo(null));
        final json = PreparedParagraphFns.toPreparedParagraphJson(r, true);
