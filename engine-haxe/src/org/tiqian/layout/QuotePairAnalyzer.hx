...
enum QuoteType { Double; Single; }
@:dataClass class QuoteRoleDecision {
    public final index:Int; public final role:FontRole; public final source:String; public final reason:String;
...
class QuotePairAnalyzer {
    ...
    public static function isNonCjkInWordApostrophe(text:String,index:Int):Bool {
        ...
        final after=codePointAtOrNull(text,index+1); if(after==null)return false;
        return isNonCjkWordCharacter(before) && isNonCjkWordCharacter(after) && (isNonCjkNonNumericWordCharacter(before) || isNonCjkNonNumericWordCharacter(after));
    }
    public static function isDigitBoundClosingQuote(text:String,index:Int):Bool return (text.charCodeAt(index)==0x2019 || text.charCodeAt(index)==0x201D) && codePointBefore(text,index)!=null && UnicodeNumber.contains(codePointBefore(text,index));
    public static function isNonCjkWordInternalQuotePair(text:String,pair:QuotePair):Bool {
        final before=codePointBefore(text,pair.openIndex); final after=codePointAtOrNull(text,pair.closeIndex+1);
        if(before==null || after==null || !isNonCjkNonNumericWordCharacter(before) || !isNonCjkNonNumericWordCharacter(after)) return false;
        var index=pair.openIndex+1; while(index<pair.closeIndex) { final cp=codePointAtOrNull(text,index); if(cp==null || !isNonCjkWordCharacter(cp)) return false; index += cp>0xFFFF ? 2 : 1; } return true;
    }
    static function isNonCjkWordCharacter(cp:Int):Bool return UnicodeWordCharacter.contains(cp) && UnicodeScriptEvidenceClassifier.classify(cp)!=UnicodeScriptEvidence.EastAsian;
    static function isNonCjkNonNumericWordCharacter(cp:Int):Bool return isNonCjkWordCharacter(cp) && !UnicodeNumber.contains(cp) && !isFullwidthEastAsianWidth(cp);
    static function isFullwidthEastAsianWidth(cp:Int):Bool return cp==0x3000 || (cp>=0xFF01&&cp<=0xFF60) || (cp>=0xFFE0&&cp<=0xFFE6);
    public static function codePointBefore(text:String,index:Int):Null<Int> { if(index<=0)return null; final low=text.charCodeAt(index-1); if((low<0xDC00||low>0xDFFF)||index<2)return low; final high=text.charCodeAt(index-2); if(high<0xD800||high>0xDBFF)return low; return 0x10000+((high-0xD800)<<10)+(low-0xDC00); }
    public static function codePointAtOrNull(text:String,index:Int):Null<Int> { if(index<0||index>=text.length)return null; final high=text.charCodeAt(index); if(high<0xD800||high>0xDBFF||index+1>=text.length)return high; final low=text.charCodeAt(index+1); if(low<0xDC00||low>0xDFFF)return high; return 0x10000+((high-0xD800)<<10)+(low-0xDC00); }
}
