...
class LineAdjustmentStageJfTestSupport {
    public static function resolver(p:ClreqProfile):ClreqProfileResolver return new FixedClreqResolver(p);
    public static function engine(?shaper:ITextShaper, hyphenate:Bool, p:Null<ClreqProfile>):ExplainableStubParagraphLayoutEngine {
        ...
        return shaper != null ? new ExplainableStubParagraphLayoutEngine(null,null,r,null,null,null,null,null,null,null,shaper)
            : hyphenate ? new ExplainableStubParagraphLayoutEngine(null,null,r,null,null,null,null,null,null,null,null,EnglishHyphenation.enUs())
            : new ExplainableStubParagraphLayoutEngine(null,null,r);
    }
    public static function layout(text:String,width:Float,?objects:Array<InlineObjectSpan>,?hyphenate:Bool=false,?shaper:ITextShaper,?profile:ClreqProfile):LayoutResult {
        final e=LineAdjustmentStageJfTestSupport.engine(shaper,hyphenate,profile);
        return e.layout(new LayoutInput(new TiqianTextContent(text),null,new ParagraphStyle(null,null,null,Ic.Zero),new LayoutConstraints(width),null,null,null,null,objects));
    }
}
class FixedClreqResolver implements ClreqProfileResolver {
    final profile:ClreqProfile; public function new(p:ClreqProfile) profile=p;
    public function resolve(id:LayoutProfileId):ClreqProfile return profile;
}
class DashBoundsShaper implements ITextShaper {
    final delegate:ExplainableStubTextShaper; final wide:Bool;
    public function new(wide:Bool) { delegate=new ExplainableStubTextShaper(); this.wide=wide; }
