...
class ExplainableStubParagraphLayoutEngineTestSupport {
    public static function start(name:String):TestTraceRecorder { var t=new TestTraceRecorder("ExplainableStubParagraphLayoutEngineTest"); t.section(name); return t; }
    public static function textLength(s:String):Int return s.length;
    public static function graphemeBoundaries(s:String):Array<Int> return SourceInteractionBoundaries.sourceGraphemeBoundaries(s,new TextRange(0,s.length));
    public static function renderRanges(a:Array<TextRange>):String { var s="["; for(i in 0...a.length) { if(i>0)s+=", "; s+=Std.string(a[i]); } return s+"]"; }
    public static function renderPairs(a:Array<String>, b:Array<String>):String { var s="["; for(i in 0...a.length) { if(i>0)s+=", "; s+="("+a[i]+", "+b[i]+")"; } return s+"]"; }
    public static function input(text:String, width:Float, ?style:ParagraphStyle, ?shaper:ITextShaper, ?textStyle:TextStyle):LayoutInput {
        return new LayoutInput(new TiqianTextContent(text), textStyle, style == null ? new ParagraphStyle(null,null,null,Ic.Zero) : style, new LayoutConstraints(width));
    }
    public static function engine(?shaper:ITextShaper, ?breaker:LineBreaker):ParagraphLayoutEngine.ExplainableStubParagraphLayoutEngine {
        return new ParagraphLayoutEngine.ExplainableStubParagraphLayoutEngine(null,null,null,null,null,null,null,null,breaker,null,shaper,new NoHyphenator());
    }
    public static function renderStrings(a:Array<String>):String {
        var s="["; for (i in 0...a.length) { if(i>0)s+=", "; s+="'"+a[i]+"'"; } return s+"]";
    }
}
class EmptyTextShaper implements ITextShaper {
    public function new() {}
    public function shape(input:ShapingInput):ShapingResult return new ShapingResult([],[]);
}
