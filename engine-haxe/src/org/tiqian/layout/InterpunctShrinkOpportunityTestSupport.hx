...
class InterpunctShrinkOpportunityTestSupport {
    public static function haltInkShaper():ITextShaper return new InterpunctHaltShaper();
    public static function preserveResolver():ClreqProfileResolver return new InterpunctPreserveResolver();
}
...
class InterpunctPreserveResolver implements ClreqProfileResolver {
    ...
    public function resolve(profileId:org.tiqian.core.LayoutProfileId):ClreqProfile {
        return new ClreqProfile(ClreqProfile.MainlandHorizontal.id, ClreqStrictness.Normal, ClreqRegion.Mainland,
            CjkPunctuationGlyphPolicy.PreserveInput, null, null, null, new AdjustmentStylePolicy(),
            KinsokuMode.MeasureAdaptive(14.0, 24.0, 32.0), new PunctuationWidthPolicy());
    }
}
    ...
    public function shape(input:ShapingInput):ShapingResult {
        ...
        for (ri in 0...res.glyphRuns.length) {
            ...
            for (gi in 0...run.glyphs.length) {
                ...
                glyphs.push(new Glyph(g.id, g.clusterRange, g.advance, g.x, g.y, g.renderFontKey, bounds,
                    (inter || ellipsis) ? 8.0 : g.haltAdvance, inter ? -4.0 : ellipsis ? 0.0 : g.haltPlacementX));
            }
