...
class LineAdjustmentStageCoverageTest {
    ...
    @:test public static function attachedObjectMarkHangsInsteadOfLeavingTheSeparatorAtAnEdge():Void {
        ...
        final r = LineAdjustmentStageCoverageTestSupport.layout(text, 48.0, null,
            [new InlineObjectSpan(new TextRange(1, 2), 100.0, 12.0, 12.0)]);
        var hung = r.lines[0];
        for (i in 0...r.lines.length) if (r.lines[i].hangingPunctuationAdvance > 0.0) { hung = r.lines[i]; break; }
        ...
        for (i in 0...r.debug.lineEdgeTrimDecisions.length) if (r.debug.lineEdgeTrimDecisions[i].reason == "LineEdgeWordSpaceCollapse") { noneCollapse = false; break; }
    ...
    @:test public static function blankMiddleLineSkipsEveryEdgePass():Void {
        ...
        for (i in 0...r.debug.justificationDecisions.length) {
            ...
            if (d.lineRange.start == r.lines[1].range.start && d.lineRange.end == r.lines[1].range.end) { noneJust = false; break; }
        }
    ...
    @:test public static function dashRunWithoutInkBoundsKeepsSyntheticGlyphs():Void {
        ...
        var allNull = true;
        for (i in 0...r.glyphRuns[0].glyphs.length) if (r.glyphRuns[0].glyphs[i].bounds != null) { allNull = false; break; }
    ...
    @:test public static function formulaObjectWithoutBoundaryDiscardsNothingAtLineEnd():Void {
        ...
        final text = "\u7532" + InlineObjectSpan.INLINE_OBJECT_REPLACEMENT_CHAR + "\u4E59\u4E19\u4E01\u620A";
        final r = LineAdjustmentStageCoverageTestSupport.layout(text, 48.0, null,
            [new InlineObjectSpan(new TextRange(1, 2), 24.0, 12.0, 12.0)]);
        TracedAssertions.assertEqualsIntRange(new IntRange(0, 1), r.lines[0].clusterRange, LineAdjustmentStageCoverageTestSupport.renderLines(r.lines));
        var noneDiscard = true;
        for (i in 0...r.debug.lineEdgeTrimDecisions.length) if (r.debug.lineEdgeTrimDecisions[i].reason == "InlineObjectLineEndDiscardableGlue") { noneDiscard = false; break; }
    ...
    @:test public static function hyphenSqueezeConsumesTheWordSpaceRawAdvanceChannel():Void {
        ...
        for (i in 0...r.clusters.length) if (r.clusters[i].text == " ") { space = r.clusters[i]; break; }
    ...
    @:test public static function hyphenSqueezeFallsBackToZeroUsedGlueWhenTheLineAlreadyFits():Void {
        ...
        TracedAssertions.assertEqualsIntRange(new IntRange(0, 3), bracket.lines[0].clusterRange, LineAdjustmentStageCoverageTestSupport.renderLines(bracket.lines));
    ...
    @:test public static function loneLatinClusterMergesBothAutoSpaceEdgeTrimsIntoOneKey():Void {
        ...
        for (i in 0...trims.length) sides.push(trims[i].side);
        TracedAssertions.assertEqualsStringArray(["trailing", "leading"], sides);
        var allMatch = true;
        for (i in 0...trims.length) {
            final d = trims[i];
            if (!(d.clusterRange.start == 1 && d.clusterRange.end == 2 && d.trimAmount == 2.0)) { allMatch = false; break; }
        }
    ...
    @:test public static function loneMandatoryBreakEmitsTwoZeroWidthLines():Void {
        ...
        var allZero = true;
        for (i in 0...r.lines.length) if (!(r.lines[i].naturalWidth == 0.0 && r.lines[i].visualWidth == 0.0)) { allZero = false; break; }
    ...
    @:test public static function mandatoryBreakMiddleLineSkipsItsJustificationPlan():Void {
        ...
        for (i in 0...r.lines.length) if (!(r.lines[i].adjustedWidth == r.lines[i].naturalWidth)) { allAdjusted = false; break; }
    ...
    @:test public static function technicalLineBodyStretchRejectsTheCleanTierAndReplays():Void {
        ...
        for (i in 0...reasons.length) if (StringTools.startsWith(reasons[i], "CurrentLineTechnicalTierRejection:")) { hasRejection = true; break; }
        ...
        for (i in 0...bReasons.length) if (bReasons[i] == "CurrentLineTechnicalEmergencyBreak") { hasEmergency = true; break; }
    ...
    @:test public static function tinyTechnicalTrackingStaysBelowTheRejectionThreshold():Void {
        ...
        var allSmall = true;
        for (i in 0...deltas.length) if (!(deltas[i] <= 0.001)) { allSmall = false; break; }
        ...
        for (i in 0...tinyReasons.length) if (StringTools.startsWith(tinyReasons[i], "CurrentLineTechnicalTierRejection:")) { noneRejection = false; break; }
        ...
        for (i in 0...rejReasons.length) if (rejReasons[i] == "CurrentLineTechnicalTierRejection:WholeToken") { hasWholeToken = true; break; }
    ...
    @:test public static function zeroAdvanceEdgeSpaceIsNeverCollapsed():Void {
        ...
        TracedAssertions.assertTrue(LineAdjustmentStageCoverageTestSupport.isAllSpaces(edge.text), LineAdjustmentStageCoverageTestSupport.renderClusterTexts(r.clusters));
