...
class LineGeometryStageFns {
 public static function resolveLineVerticalGeometry(input:LayoutInput,fontSize:Float,pinyinSpans:Array<RubySpan>,naturalClusters:Array<Cluster>,lineSolution:LineSolution,rubyFontGeometryBySpan:Map<RubySpan,RubyFontGeometry>,existingInterlineSpace:Float,baseLineMetrics:ResolvedLineMetrics,baseFaceHeight:Float,rubyExtent:Float,inlineObjectByClusterIndex:Map<Int,InlineObjectSpan>,baseAscent:Float,baseDescent:Float):LineVerticalGeometryStageResult {
  final ext:Array<Float>=[]; for(line in lineSolution.lines){var x=0.0;for(ruby in pinyinSpans){final range=clusterIndexRangeFor(naturalClusters,ruby.baseRange);if(range!=null&&range.start<=line.clusterRange.end&&range.end>=line.clusterRange.start&&rubyFontGeometryBySpan.exists(ruby))x=Math.max(x,rubyFontGeometryBySpan.get(ruby).requiredExtent);}ext.push(x);}
  ...
  final rd=pinyinSpans.length==0?null:new RubyLineHeightDecisionInfo(input.paragraphStyle.rubyLineHeightMode.name,baseLineMetrics.height,baseFaceHeight,rubyExtent,existingInterlineSpace,extras.length==0?0:extras[0],extras,[],extras.any(x -> x>0)?"ConditionalRubyLineHeight":"ExistingInterlineSpaceFitsRuby");
