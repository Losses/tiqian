...
class RubyLayoutTestSupport {
    ...
    public static function input(ruby:Array<RubySpan>):LayoutInput {
        return new LayoutInput(new TiqianTextContent("\u4E2D\u6587\u6392\u7248"), null,
            new ParagraphStyle(null, null, null, new Ic(0)), new LayoutConstraints(400.0), null, null, ruby);
    }
    ...
    public static function layoutUniform(ruby:Array<RubySpan>):LayoutResult {
        return engine().layout(new LayoutInput(new TiqianTextContent("\u7532\u4E59\u4E19\u4E01\u620A\u5DF1\u5E9A\u8F9B\u58EC\u7678\u5B50\u4E11"), null,
            new ParagraphStyle(null, null, 18.0, new Ic(0), null, null, null, RubyLineHeightMode.UniformParagraph),
            new LayoutConstraints(64.0), null, null, ruby));
    }
    public static function layoutContradictory(reading:String):LayoutResult {
        ...
        return e.layout(new LayoutInput(new TiqianTextContent("\u7532\u4E59\u4E19\u4E01"), null,
            new ParagraphStyle(null, null, 18.0, new Ic(0)), new LayoutConstraints(64.0), null, null,
            [new RubySpan(new TextRange(0, 1), reading)]));
    }
    public static function totalWidth(texts:Array<String>):Float {
        final spans:Array<RubySpan> = [];
        for (i in 0...texts.length) spans.push(new RubySpan(new TextRange(i, i + 1), texts[i]));
        final r = engine().layout(new LayoutInput(new TiqianTextContent("\u4E2D\u6587\u6392\u7248"), null,
            new ParagraphStyle(null, null, null, new Ic(0)), new LayoutConstraints(4000.0), null, null, spans));
        var total = 0.0;
        for (i in 0...r.clusters.length) total += r.clusters[i].advance;
        return total;
    }
    public static function assertFloatListEquals(expected:Array<Float>, actual:ReadOnlyArray<Float>):Void {
        final e:Array<Int> = [];
        for (i in 0...expected.length) e.push(Std.int(expected[i]));
        final a:Array<Int> = [];
        for (i in 0...actual.length) a.push(Std.int(actual[i]));
...
class ContradictoryInkShaper implements ITextShaper {
    ...
    public function shape(input:ShapingInput):ShapingResult {
        ...
        for (i in 0...r.decisions.length) {
            ...
            ds.push(new ShapingDecisionInfo(d.range, d.sourceText, d.displayText, d.fontKey, d.glyphCount, d.advance, d.source, d.reason, 0,
                d.missingGlyphs, d.resolvedFace, d.script, d.language, d.strategy, d.featureEvidence, d.capabilityIssue));
        }
